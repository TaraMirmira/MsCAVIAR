"""
This script takes plink LD lists and simulates summary statistics for multiple populations with sample size imbalance. 
Requires 2 populations, can take up to 4.
"""
import argparse
    ...
    if study == 0: # first study should always be significant
      ...
      for i in range(self.num_snps):
        if lambda_c[i] != 0: # causal snp
          if p_list[i] <= 5e-8:
            ...
            # print(p_list[i])
            return False
        else: # non-causal snps should be significant more than 5% of the time (not too easy)
          if p_list[i] <= 5e-8:
            count_sig += 1
    else: # other studies
      # print("study #", study)
      for i in range(self.num_snps):
        if lambda_c[i] != 0: # causal snp
          ...
          if p_list[i] <= 5e-8:
            count_sig += 1
    if (count_sig / self.num_snps) < 0.05:
      return False
    ...
    for i in range(len(self.ld)):
      ...
      while l < (self.sims + 1):
        ...
        if self.test(lambda_c, p, i):
          ...
          p = open(p_name,'w')
          for k in range(self.num_snps): # for paintor's input, generate required information
